...
bool XMLScene::parseNode(TiXmlElement *curr_node, bool is_inside_dl) {
	...
	if (curr_node->QueryBoolAttribute("isselectable", &is_selectable)
			!= TIXML_SUCCESS) {
		printf("No \"isselectable\" attribute\n");
	}
	...
	while ((child = (TiXmlElement *) children->IterateChildren(child))) {
		...
		if (strcmp(child_type, "rectangle") == 0) {
			...
			if (is_inside_dl || is_dl) {
				printf(
						"Waterline defined in \"%s\" is inside a display list!\n",
						node_id);
				throw InvalidXMLException();
			}
			...
			if (strdup(frag_shader, child->Attribute("fragmentshader")) == NULL) {
				printf(
						"Error parsing \"fragmentshader\" attribute of waterline!");
				throw InvalidXMLException();
			}

			if (strdup(vert_shader, child->Attribute("vertexshader")) == NULL) {
				printf(
						"Error parsing \"vertexshader\" attribute of waterline!");
				throw InvalidXMLException();
			}

			MyWaterLine *wl = new MyWaterLine(heightmap, texturemap,
